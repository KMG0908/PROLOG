...
public class SixHour extends TimerTask{
	private String email;
	public long size1 = 0;
	public long size2 = 0;
	
	private String url = DBConnection.URL + "/Alert";
	private String user = "root";
	private String pw = "1234";

	@Override
	public void run() {
		System.out.println(new Date() + "	SixHour!	");
	  		...
	  		if(dirList.length != 0){
	  			...
	  			if(subDirList.length != 0){
					...
					defaultFile = new File(filePath);
	  				if(size2 == 0){
	  					size2 = subDirList[subDirList.length-1].length();
	  				}
	  				else{
	  					size1 = size2;
	  					size2 = subDirList[subDirList.length-1].length();
	  					rate = (double)size2 / size1;
	  				}
	  			}
	  		}
	  	}
	    ...
	    if(rate >= 10){
			...
			int checkedLog = dao.getRowNum("select * from " + getEmail() + "alert.userLog");
			int detectedLog = dao.getRowNum("select * from " + getEmail() + "alert.warningLog");
			int danger1 = dao.getRowNum("select * from " + getEmail() + "alert.warningLog where AttackLevel='1' or AttackLevel='2' or AttackLevel='3'");		        
			int danger2 = dao.getRowNum("select * from " + getEmail() + "alert.warningLog where AttackLevel='4' or AttackLevel='5' or AttackLevel='6'");
			
			if(detectedLog != 0){
				int rowNum = dao.getRowNum("select * from alert.alert where Email='" + getEmail() + "'");
			    
			    if(rowNum != 0){
			    	dao.deleteAlret(getEmail());
			    }
